...
| 构建信息在生产产物整列消失（版本号、分支、时间显示 `unknown`） | 用 **HTML 内联 `<script>`** 注入构建信息，撞 MV3 CSP（无 `'unsafe-inline'`）被拦 | 见 [ARCHITECTURE.md](../../../ARCHITECTURE.md)「构建信息注入」——**一律走 `vite.define` 裸标识符**，别再引入内联注入 |
