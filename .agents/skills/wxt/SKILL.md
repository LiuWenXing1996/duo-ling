...
3. **同一名字不得同时存在 `x.html` 与 `x.ts`** —— WXT 判定两个同名 entrypoint（如 `sidepanel.html` 与 `sidepanel.ts`）。**入口脚本一律用非约定名**（如 `app/sidepanel-main.ts`）由 html 引用；脚本名一旦撞上 html 的 basename 就会被当成第二个入口。
