...
import { writeToolPage, newToolScaffoldHtml, toolsRoot, previewRoot, listToolPages, updateToolMeta, applyToolChanges, deleteToolPage, type ToolChangeList } from './tool-page'
import {
  initToolRepo,
  commitToolChanges,
  listToolHistory,
...
function buildGeneratorSystemPrompt(): string {
  ...
  return [
    ...
    '工具目录里只有两个可改文件：index.html（工具页面主体）、meta.json（工具元信息 name / title / description / icon）。',
    ...
    '2. write meta.json：{"op":"write","file":"meta.json","content":{"name":"kebab-case-id","title":"工具名","description":"说明","icon":"图示字符"}}。其中 icon 可选，必须是单个字符（emoji 或字母/汉字/符号，如 "🗂" 或 "文"），用于工具卡片与标签展示，缺省时不填。',
...
app.whenReady().then(() => {
  ...
  ipcMain.handle(
    ...
    ): { ok: boolean; title?: string; icon?: string; error?: string } => {
