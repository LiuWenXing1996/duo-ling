...
// —— 版本预览边栏 ——
// 桌面版：主进程把目标 commit 的整棵树物化到缓存区，再由 <webview> 加载 tool-preview://<id>/<oid>/；
// 扩展版：没有可渲染的文件 URL，改为取该提交下的工具页 HTML（`html`），用 sandbox iframe 的 srcdoc 渲染。
// 每个预览位：单版本 1 个，并排对比 2 个
interface PreviewItem {
  oid: string
  /** 扩展版无文件 URL，恒为空串（保留字段以对齐桌面版形状） */
  url: string
  /** 扩展版：目标提交下的工具页 HTML，交给 iframe 的 srcdoc */
  html: string
...
// 取某个 commit 的工具页 HTML（扩展版以 srcdoc 渲染，无 URL）
async function materialize(
  oid: string
): Promise<Pick<PreviewItem, 'url' | 'html' | 'status' | 'error'>> {
  try {
    const res = await window.api.tool.preview(props.toolId, oid)
    if (res.ok) return { url: res.url, html: res.html ?? '', status: 'ok', error: '' }
    return { url: '', html: '', status: 'error', error: res.error }
  } catch (e) {
    return { url: '', html: '', status: 'error', error: e instanceof Error ? e.message : String(e) }
  }
}
      ...
      >
        <section v-for="item in previewItems" :key="item.oid" class="tool-history__preview-col">
          ...
          <div class="tool-history__preview-frame">
            <!-- 扩展版载体：srcdoc + sandbox iframe（桌面版为 <webview :src="tool-preview://...">）。
                 allow-same-origin 是必要的：srcdoc 内联脚本被扩展 CSP 拦，工具页只能靠同源的
                 /tool-bridge.js 跑起来；分层沙箱（不可信页放 opaque origin）见 README「工具页承载」。 -->
            <iframe
              v-if="item.html"
              :srcdoc="item.html"
              class="tool-history__webview"
              sandbox="allow-scripts allow-same-origin allow-forms allow-modals"
              @load="onPreviewDomReady(item.oid)"
            />
