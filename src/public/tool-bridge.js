...
(function () {
  ...
  // 桥接契约（宿主侧 ToolFrame 路由 → background）：
  //   run(capId, input) → 能力产物
  //   gitCommit(message) → { committed: boolean, oid?: string }；内容无净变更时 committed=false，
  //                        调用方应据此提示「无变更」，而不是一律提示已提交。
  window.cap = {
  ...
  if (runBtn) {
    runBtn.onclick = async () => {
      ...
      try {
        ...
        const res = await window.cap.gitCommit('render')
        out.insertAdjacentHTML(
          'beforeend',
          res && res.committed
            ? '<div style="color:#0a0">已提交版本</div>'
            : '<div style="color:#a50">内容无变化，未产生新版本</div>'
        )
      } catch (err) {
