...
export function buildSystemPrompt(
  ...
  const lines = [
    ...
    '脚本跑在页面主世界：能不能用 eval / new Function 取决于**目标站点自己的 CSP**，本扩展不拦，',
    '故不要依赖内部靠动态代码生成的库（如 ajv 的编译校验器、Vue 运行时模板编译器）——',
    '站点一旦收紧 CSP，这类代码会在目标页静默失败，很难排查。需要动态行为就用普通函数、事件监听或数据驱动写法。',
