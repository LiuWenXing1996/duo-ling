 ...
 */
export const OFFSCREEN_KIND_PREFIXES = ['ai:', 'state:'] as const
...
chrome.runtime.onMessage.addListener((raw, _sender, sendResponse): boolean => {
  ...
  // 异步应答的命令面前缀：ai: 是 git 历史（ai:build 单独走构建命令面），state: 是项目状态库
  // 的写侧（单写方）。三者都 return true —— 告诉 chrome.runtime 我们要异步 sendResponse
  // （否则响应会被丢弃）。
  const kind = msg?.kind
