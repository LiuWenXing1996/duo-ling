> ## ⚠️ 归档说明
>
> 本目录是哆灵**原 Electron 桌面版**的代码归档，**仅为迁移期间的只读参照**：
> **不参与构建**（根工程只编译 `entrypoints`/`src`/`.wxt`）、**不要在此改代码**、**迁移完成后可整目录删除**。
> 需要迁移的逻辑请平移到根 `src/`。归档内容清单见 [ARCHIVE.md](ARCHIVE.md)。
>
> 以下为归档时的原始 README，保留备查。
