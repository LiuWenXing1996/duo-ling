...
`meta.json.icon` 接受两种格式（二选一，主进程 `normalizeToolIcon` 归一化）：

| 格式 | 示例 | 说明 |
| --- | --- | --- |
| 单个字符 | `📄` / `文` / `A` | emoji / 字母 / 汉字，按码点计 1；渲染层直接显示 |
| `lucide:<名称>` | `lucide:file-text` | lucide 图标名（kebab-case），渲染层从允许列表按需动态加载；不在列表的名称渲染时回退名称首字符 |

- 留空或不合法时回退为工具名首字符（再兜底 `✨`）。
- lucide 可用名称见渲染层 `src/renderer/src/lib/lucide-icons.ts` 的允许列表（新增图标在该文件的 glob 模式追加）。
