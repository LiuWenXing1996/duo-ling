// DL.cookie 手测探针（get / set / remove + 域名门）。
// 用法：导入本包 → 启用 → 打开 https://example.com（**匹配规则刻意只写 example.com**，
// 探针的越域用例要依赖「脚本作用域之外」才有意义；换站前先改 matches）。
// 效果：页面右下角出现角标，点击后逐项跑用例并就地标 ✓ / ✗。
// 用例全程自清理（写入的 cookie 末尾删掉），跑完页面 cookie 不留痕。
;(async () => {
  var ID = 'dl-test-cookie'
  var COOKIE = 'dl_cookie_probe'
  ...
  // ① set → get 往返：url 缺省（包装层填 location.href）+ document.cookie 交叉验证
  async function roundTripCase() {
    try {
      await DL.cookie.set({ name: COOKIE, value: 'v1', secure: true })
      var all = await DL.cookie.get()
      if (!isArray(all)) {
        line('set → get 往返（url 缺省当前页）', false, 'get() 没返回数组：' + JSON.stringify(all))
        return
      }
      var one = all.filter(function (c) { return c.name === COOKIE })[0]
      if (!one) {
        line('set → get 往返（url 缺省当前页）', false, '写的 cookie 读不回来')
        return
      }
  ...
  async function byNameCase() {
    try {
      var hit = await DL.cookie.get({ name: COOKIE })
      var miss = await DL.cookie.get({ name: COOKIE + '_nope' })
  ...
  async function pathAgnosticCase() {
    try {
      var r = await DL.cookie.get({ url: location.origin + '/some/deep/path?q=1' })
  ...
  async function outOfScopeCase() {
    try {
      await DL.cookie.get({ url: 'https://out-of-scope.test/' })
  ...
  async function badUrlCase() {
    try {
      await DL.cookie.get({ url: 'about:blank' })
  ...
  // ⑥ remove 后读不到（同一 name 写两条路径无关，这里只验「删了就没了」）
  async function removeCase() {
    try {
      await DL.cookie.remove({ name: COOKIE })
      var after = await DL.cookie.get({ name: COOKIE })
      var gone = isArray(after) && after.length === 0
      var pageGone = ('; ' + document.cookie).indexOf('; ' + COOKIE + '=') < 0
      line('remove 后读不到', gone && pageGone, 'DL 读 ' + after.length + ' 条 / 页面可见=' + !pageGone)
    } catch (e) {
      line('remove 后读不到', false, code(e) + ' ' + msg(e))
    }
  }
  ...
  try {
    if (!window.DL || !window.DL.cookie) {
      render('DL_MISSING（本脚本世界没有 DL.cookie）')
      return
    }
    render('DL.cookie 探针 · 点击运行（需在 https://example.com 上）')
    DL.log('探针就绪，点击角标开始')
  } catch (e) {
