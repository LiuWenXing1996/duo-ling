...
- 用户脚本启用 / 停用 / 删除与「本页在跑哪些脚本」改由 Violentmonkey 内核驱动：装哪一个、注入到哪些页面都走脚本声明的匹配规则与 `@run-at`

### Changed

- 脚本的执行底座整套换成 Violentmonkey 内核（内置一份开源的标准实现，随构建产出）：自研 GM 运行时与旧桥接层删除，标准油猴脚本的行为对齐随之提升
- 移除三个哆灵独有的 GM 成员 `GM.clearValues` / `GM.focusTab` / `GM.page.*`（含 `GM.page.listen` / `GM.page.fetchHook`）：用到它们的脚本需要改写，这几项在标准里没有对应物，新内核不提供支持
- GM 能力清单与「GM API 速查」只列实际支持的成员：此前在列表里、但既不在标准也不在本扩展实现里的条目移除
- 脚本是否注入只看是否启用，不再因为删除残留而被放过

### Fixed

- `GM_setClipboard` 此前静默不生效（命令没注册），现在能正常写入剪贴板
- 脚本的数据投递改回标准注入方式，不再触发 Chrome 拒绝自定义世界对象导致的注入失败
- 右键菜单层级收敛，不再层层嵌套
- 离屏文档不再重复注入运行时脚本

### Changed (tooling)

- 包管理器改用 pnpm（含 workspace，`packages/gm-runtime` 纳入工作区管理），命令面引用同步更新
- 新增 gm-runtime 内部包：固定 Violentmonkey v2.49.0 源码，跑它的构建链产出库产物与注入件
- 端测改为先拉钉住版本的 VM 源码再构建；4 个受 VM 集成影响的已知失败用例先挂起并登记待办
