...
- **存储**：项目数据（源码/配置/产物/enabled）在**独立 IndexedDB 库 `duoling-state`**（`state-db.ts` / `project-store.ts` 读、`project-write.ts` 写，**写只归 offscreen**，见 `notes/content/userscript-single-writer.md`）；`chrome.storage.local` 只剩 `DL.store` 值（`us:gm:*`）与错误日志（`us:errors`）；`lightning-fs`（IndexedDB 后端，库名 `duoling`，**只许 offscreen 碰**）存脚本 git 历史与会话 `duoling-chat`
