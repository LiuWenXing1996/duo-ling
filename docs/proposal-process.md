...
| 收件箱条目的增删与分区迁移 | `docs/inbox.md`「待办」「不办」两分区下的条目增删、「待办」↔「不办」互移 | 防止自锁：收件箱专收「还没有方案的问题」，要求每条走提案等于要求想法一诞生就得有方案，收件箱随之失效    |
...
- PR：一次变更就是一次 PR
  ...
  - PR 与提案
    ...
    - 提案自身的创建、内容修改与状态流转都不是豁免项：它们属于该提案那个 PR 的一部分（从落 `draft/` 起到实施完成，全在同一个 PR 里）
...
- **状态变更**：
  ...
  - **必须单独一个 commit**
    ...
    - 与 PR 的关联
      - 实施中 → 实施完成：属非豁免类 commit，只能与提案类 PR 关联，且状态变更 commit 必须是它的**最后一个** commit
