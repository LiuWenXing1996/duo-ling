...
**背景**：测试体系空缺——原 Vitest/Playwright 配置随 Electron 归档到 `legacy/`，本工程目前只有 `typecheck` 一道静态关卡。2026-09-15 与老大收敛定稿。
...
**E2E 关键结论（2026-09-15 核查 WXT `guide/essentials/e2e-testing` 与 Playwright `docs/chrome-extensions`）**：
...
**顺序建议**：1️⃣2️⃣ 首批 → E2E 冒烟 → 3️⃣ → 4️⃣ 按需。

**状态**：方案已过稿（2026-09-15），待开工。
