...
- 前提：确认旧数据为空（全量用户已是新 schema）。在此前提下，以下四处理旧数据兼容分支成为死代码，永不触发：
  ...
  4. **旧 GM 形态脚本记录识别/清理** —— 横跨多文件：判别函数 `isLegacyScriptRecord` 连同其专有 `UserScriptMeta` 类型（`src/lib/userscripts/types.ts`）、列表扫描 `listLegacyScripts` 与一键清理 `clearDeprecatedScripts`（`src/lib/userscripts/store.ts`，后者经 background 的 `userscript:clearDeprecated` 处理器暴露给 UI）、以及 `listSummaries` 拼接旧记录的 legacy 段。GM 旧形态已弃用，这类记录不再产生，整条链路是死代码。
...
- **`src/lib/userscripts/types.ts`**：删 `UserScriptMeta` 接口与 `isLegacyScriptRecord` 函数（含其注释）；`deprecated` 识别失去数据源。
...
- **测试**：`types.test.ts` 删 `isLegacyScriptRecord` describe；`store.test.ts` 删 `listLegacyScripts` / `clearDeprecatedScripts` 两个 describe，并修正 `listSummaries` 用例（去掉 deprecated 旧记录断言）；`extension-ipc.test.ts` 删 `userscript:clearDeprecated` 用例。
...
- [ ] 全仓检索 `LEGACY_SEQ_KEY` / `isLegacyScriptRecord` / `listLegacyScripts` / `clearDeprecatedScripts` / `clearDeprecated` / `UserScriptMeta` / `migrated` 不再命中（注：`SEQ_META_KEY` 与新数据 `conversationSeq` 键保留，非检索目标）
...
- 不删 `UserscriptListPanel.vue` 里消费 `ScriptSummary.deprecated` 的展示分支（灰显 / 「含 N 个已弃用旧记录」提示 / 删除弹窗注释）。旧数据源删后该分支恒为假、永不触发，属无害死代码；本次不动 UI，避免扩大改动面，留作后续单独清理（届时一并移除 `deprecated` 字段）
