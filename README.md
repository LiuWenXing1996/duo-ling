...
20. **GM 可用性矩阵（全量 API 自检）**：`npm run pack:uscripts` → 导入 → 启用「GM 可用性矩阵」→ 打开任意 http(s) 页面 → 点右下角角标跑全部（约 10s）→ 面板出三态明细，同时 `console.log` 一份可整段复制的文本。
    - **判读**：`✗` 才是真问题（某条 API 在真机上不通）；`?` 是环境原因、别当 bug —— 网络不可达 / 页面 CSP 拦了注入的 script / 需要人工耳目（剪贴板回读、菜单项是否出现）。所以尽量挑**无严格 CSP** 的站点跑，`GM.page.*` 与 `window.onurlchange` 那三条才跑得满。
    - **两条会先弹 confirm**：下载（往下载目录落 2 个 html）与 GM_cookie 写入（写一条探针 cookie 后立刻删）。
