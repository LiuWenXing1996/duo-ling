...
20. **GM 可用性矩阵（全量 API 自检）**：`npm run pack:uscripts` → 导入 → 启用「GM 可用性矩阵」→ 打开任意 http(s) 页面 → 点右下角角标跑全部 → 面板出三态明细，同时 `console.log` 一份可整段复制的文本。
    - **三项要你动手**（轮到它们时面板顶部会出现 `→ …` 提示，全程约 30–60s）：`GM.page.listen`（点页面任意处）、`GM_setClipboard`（在左下角输入框按一次 Cmd/Ctrl+V）、`GM_registerMenuCommand`（右键 → 点「GM 矩阵：点我试试」）。错过提示就是 `?`，重跑一次即可。
    - **判读**：`✗` 才是真问题（某条 API 在真机上不通）；`?` 是环境或人手原因、别当 bug —— 网络不可达 / 本页这 12s 内没发请求（`GM.page.fetchHook` 靠被动观察） / 上面三项你没动手。**`GM.page.*` 不能靠「注入内联 script」自触发**：实测在本扩展的脚本世界里注入的内联 `<script>` 不执行（与页面 CSP 无关，机制未定论，见 `dl-fetchhook-test` 头部），故这两项分别走「真点击」与「被动等页面请求」。
