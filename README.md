...
| 载体 | 角色 | 承载内容 |
| --- | --- | --- |
| **网页浮层 `floatpanel.html`**（content script 按需挂进页面的 iframe） | **对话界面**（唯一入口） | 当前标签页的会话（消息流、输入区含拾取 chip、模型选择）+ 顶栏「会话历史 / 打开工作台 / 收起」三个去处；会话 = **它所在标签页**的会话（tab 身份由 content script 经 iframe URL 传入）；页面加载时不注入，从 popup 或页面右键菜单打开 |
