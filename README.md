...
| **popup**（点工具栏图标弹出） | 配置入口（点开即用、点外即关） | 网页浮层开关（总开关 + 当前站点）+ 调出当前页的对话浮层（页面外入口之一）+ 本页脚本（在跑的脚本与报错，点脚本行跳工作台运行日志）+ 新版本提示（只在有新版本时出现）+「打开工作台」+「扩展管理页」（打开 `chrome://extensions` 并带本扩展 id，落在列表页，详情需再点一次）；在挂不了浮层的页面上说明原因 |

主流程：在网页浮层里对话描述需求 → 到工作台标签页管理脚本（新建 / 编辑 / 启停 / 看 git 历史）。浮层在页面右下角有一颗悬浮按钮；按钮被页面元素挡住、或本站点关着浮层时，可以从工具栏 popup 的「对话浮层」按钮或页面右键菜单「打开哆灵对话」调出来。标签页从对话界面顶栏的「打开工作台」按钮或 popup 的「打开工作台」打开，支持 hash 深链：`#/guide` 开引导、`#/tool/<uuid>` 直达该脚本编辑器、`#/errors/<uuid>` 打开运行日志标签页并过滤到该脚本、`#/settings` 开设置、`#/sessions` 开会话历史。
