...
2. **popup 配置面板**：点工具栏哆灵图标 → 弹出 popup（浮层总开关、当前站点开关、「打开工作台」）。在浏览器内部页（如 `chrome://extensions`）点开时，应看到一行说明「当前页面不能显示浮层…」
...
12. **页面脚本灵动岛与角标**：在命中脚本的页面上展开浮层，灵动岛应列出本页在跑的脚本与报错（点脚本行跳工作台运行日志）；生成过程中收起浮层，完成后工具栏图标应亮红色角标 `1`，重新展开浮层即清零
