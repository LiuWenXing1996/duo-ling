...
| **popup**（点工具栏图标弹出） | 配置入口（点开即用、点外即关） | 网页浮层开关（总开关 + 当前站点）+ 调出当前页的对话浮层（悬浮按钮被页面挡住、或本站没显示浮层时的页面外入口）+ 本页脚本（在跑的脚本与报错，点脚本行跳工作台运行日志）+ 新版本提示（只在有新版本时出现）+「打开工作台」+「扩展管理页」（打开 `chrome://extensions` 并带本扩展 id，落在列表页，详情需再点一次）；在挂不了浮层的页面上说明原因 |
