...
20. **GM 可用性矩阵（全量 API 自检）**：`npm run pack:uscripts` → 导入 → 启用「GM 可用性矩阵」→ 打开任意 http(s) 页面 → 点面板上的「**跑全部**」→ 面板出三态明细，**点「复制结果」**拿到可整段回帖的文本（同时也会 `console.log` 一份）。
    - **四项要你动手**，但**不阻塞看着你**：跑批照常走完，这四项先落 `⋯`，在左下角「**待你完成**」盒子里做完就自动翻成结果（不限时，10 分钟兜底）。四项是：`GM.page.listen`（点页面任意处）、`GM_setClipboard`（在盒子里的输入框按一次 Cmd/Ctrl+V）、`GM_registerMenuCommand`（右键 → 点「GM 矩阵：点我试试」）、`GM.page.fetchHook`（让页面**自己**发请求：换会拉接口的站点重跑，或在本页 DevTools Console 里执行 `fetch(location.href)`；安静页面如 example.com 会一直挂着）。
